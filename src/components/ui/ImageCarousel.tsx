...
export function ImageCarousel({ images, className = "", title }: ImageCarouselProps) {
    ...
    if (!images || images.length === 0) {
        return (
            <div className={`relative ${className} w-full h-full overflow-hidden bg-gradient-to-br from-primary-900/30 to-bg rounded-xl`}>
                {title ? (
                    <FivemScriptLogo title={title} />
                ) : (
